...
# Usage: SAP_PASSWORD='...' ./deploy.sh [--package ZNORTHWIND] [--parent ZLOCAL] [--transport NPLK...]
set -euo pipefail
...
create_table() {
  ...
  $ADT object create --type TABL/DT --name "$name" --package "$PACKAGE" \
    --description "$desc" $TRANSPORT_ARG
  $ADT source write /sap/bc/adt/ddic/tables/${name,,}/source/main \
    --file "$SCRIPT_DIR/$file" --activate $TRANSPORT_ARG
}
...
$ADT object create --type DEVC/K --name "$PACKAGE" --package "$PARENT" \
  --description "Northwind Sample Database" $TRANSPORT_ARG
...
create_table ZNW_ORDER_DETAILS "Northwind Order Details" znw_order_details.ddl
